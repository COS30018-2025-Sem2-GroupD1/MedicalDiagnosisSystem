...
"""
Database seeder script for populating MongoDB with sample data.

This script creates:
- Doctor accounts (accounts collection)
- Patient accounts (patients collection)
- Patient EMR entries (emr collection)

Usage:
    Set MONGO_USER environment variable to your MongoDB connection string:
    export MONGO_USER="mongodb://username:password@host:port/"
    
    Then run:
    python scripts/seed_database.py

For EC2 MongoDB instance:
    export MONGO_USER="mongodb://<EC2_IP>:27017/"
    python scripts/seed_database.py
"""
...
from src.data.connection import close_connection
from src.data.repositories.account import init as init_accounts, create_account
from src.data.repositories.patient import init as init_patients, create_patient
from src.data.repositories.emr import init as init_emr, create_emr_entry
from src.models.emr import EMRCreateRequest, ExtractedData, Medication, VitalSigns, LabResult
...
# Sample EMR entries - will be created after patients and doctors are set up
SAMPLE_EMR_DATA = [
    {
...
def generate_embeddings(dimension: int = 768) -> List[float]:
    ...
    In production, these would be generated using an embedding model.
    
    Args:
        dimension: The dimension of the embedding vector (default 768)
        
    Returns:
        List of float values representing the embedding
    """
    # Generate simple dummy embeddings (normalized random-like values)
...
def seed_doctors() -> List[str]:
    ...
    Returns:
        List of created doctor IDs
    """
    logger().info("Seeding doctor accounts...")
    doctor_ids = []
    ...
    for doctor_data in DOCTORS:
        try:
            ...
            logger().info(f"Created doctor: {doctor_data['name']} (ID: {doctor_id})")
        except Exception as e:
            logger().error(f"Failed to create doctor {doctor_data['name']}: {e}")
    
    logger().info(f"Successfully created {len(doctor_ids)} doctor accounts")
    return doctor_ids


def seed_patients(doctor_ids: List[str]) -> List[str]:
    """
    Seed the database with patient accounts.
    
    Args:
        doctor_ids: List of doctor IDs to assign to patients
        
    Returns:
        List of created patient IDs
    """
    logger().info("Seeding patient accounts...")
    patient_ids = []
    ...
    for i, patient_data in enumerate(PATIENTS):
        try:
            ...
            patient_id = create_patient(
                ...
                past_assessment_summary=patient_data.get("past_assessment_summary"),
                assigned_doctor_id=assigned_doctor_id
            )
            patient_ids.append(patient_id)
            logger().info(f"Created patient: {patient_data['name']} (ID: {patient_id})")
        except Exception as e:
            logger().error(f"Failed to create patient {patient_data['name']}: {e}")
    
    logger().info(f"Successfully created {len(patient_ids)} patient accounts")
    return patient_ids


def seed_emrs(patient_ids: List[str], doctor_ids: List[str]) -> int:
    """
    ...
    Returns:
        Number of EMR entries created
    """
    logger().info("Seeding EMR entries...")
    emr_count = 0
    
    # Create at least one EMR entry per patient
        ...
        if i < len(SAMPLE_EMR_DATA):
            ...
            try:
                ...
                extracted_data = ExtractedData(
                    ...
                    medications=[Medication(**med) for med in emr_data["medications"]],
                    vital_signs=VitalSigns(**emr_data["vital_signs"]) if emr_data["vital_signs"] else None,
                    lab_results=[LabResult(**lab) for lab in emr_data["lab_results"]],
                    procedures=emr_data["procedures"],
                    notes=emr_data.get("notes")
                )
                
                # Create EMR entry request
                ...
                emr_count += 1
                logger().info(f"Created EMR entry for patient {patient_id} (EMR ID: {emr_id})")
                
            except Exception as e:
                logger().error(f"Failed to create EMR for patient {patient_id}: {e}")
    
    logger().info(f"Successfully created {emr_count} EMR entries")
    return emr_count


def initialize_collections():
    """Initialize all collections with their schemas."""
    logger().info("Initializing collections...")
    
    try:
        init_accounts(drop=False)
        logger().info("✓ Accounts collection initialized")
    except Exception as e:
        logger().error(f"Failed to initialize accounts collection: {e}")
        raise
    
    try:
        init_patients(drop=False)
        logger().info("✓ Patients collection initialized")
    except Exception as e:
        logger().error(f"Failed to initialize patients collection: {e}")
        raise
    
    try:
        init_emr()
        logger().info("✓ EMR collection initialized")
    except Exception as e:
        logger().error(f"Failed to initialize EMR collection: {e}")
        raise
...
def main():
    ...
    print("=" * 60)
    print("Medical Diagnosis System - Database Seeder")
    print("=" * 60)
    ...
    if not mongo_user:
        ...
        print("  export MONGO_USER='mongodb://username:password@host:port/'")
        print("\nFor EC2 MongoDB instance:")
        print("  export MONGO_USER='mongodb://<EC2_IP>:27017/'")
        print("\nExample:")
        print("  export MONGO_USER='mongodb://172.31.0.1:27017/'")
        sys.exit(1)
    ...
    try:
        ...
        # Seed doctors
        print("\n" + "-" * 60)
        print("Step 2: Creating doctor accounts...")
        print("-" * 60)
        doctor_ids = seed_doctors()
        
        # Seed patients
        print("\n" + "-" * 60)
        print("Step 3: Creating patient accounts...")
        print("-" * 60)
        patient_ids = seed_patients(doctor_ids)
        
        # Seed EMRs
        print("\n" + "-" * 60)
        print("Step 4: Creating patient EMR entries...")
        print("-" * 60)
        emr_count = seed_emrs(patient_ids, doctor_ids)
